...
/**
 * Strands — React Bits component (TypeScript port)
 * Animated flowing light strands rendered via WebGL (ogl).
 *
 * Used in AutoFinDocs as:
 *   1. A captivating full-screen background while OCR processes a document,
 *      keeping the user visually engaged until extraction completes.
 *   2. A fluid animated backdrop inside the mobile hamburger menu.
 *   3. A premium ambient glow behind the dashboard hero.
 */
...
function buildPalette(colors: string[]): RGBTuple[] {
  const filled = colors && colors.length ? colors : ['#ffffff']
  const padded: RGBTuple[] = []
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      ;(glassProgram.uniforms.uResolution.value as number[]) = [width, height]
    }
